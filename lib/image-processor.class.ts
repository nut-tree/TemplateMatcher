...
export class ImageProcessor {
  static determineMatRectROI(img: Image, roi: Region): cv.Rect {
  ...
  static determineRegionRectROI(roi: cv.Rect): Region {
   ...
   */
  static fromImageWithAlphaChannel = async (img: Image, roi?: cv.Rect): Promise<cv.Mat> => {
    let mat: cv.Mat;
  ...
  static fromImageWithoutAlphaChannel = async (img: Image, roi?: cv.Rect): Promise<cv.Mat> => {
    let mat: cv.Mat;
    if (img.colorMode === ColorMode.RGB) {
